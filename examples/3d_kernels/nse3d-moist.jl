...
function volume_grad!(::Val{dim}, ::Val{N}, rhs::Array, Q, vgeo, D, elems) where {dim, N}
    ...
    @inbounds for e in elems
        for k = 1:Nq, j = 1:Nq, i = 1:Nq
            ...
            ρ, E = Q[i, j, k, _ρ, e], Q[i, j, k, _E, e]
            u, v, w = U/ρ, V/ρ, W/ρ
            IE = E/ρ - 0.5*ρ*(u^2 + v^2 + w^2) - gravity * z
    ...
    @inbounds for e in elems
        for f = 1:nface
            for n = 1:Nfp
                ...
                uM, vM, wM = UM/ρM, VM/ρM, WM/ρM
                zM = vgeo[vidM, _z, eM]
                IEM = EM/ρM - 0.5*ρM*(uM^2 + vM^2 + wM^2) - gravity * zM
                ...
                if bc == 0
                    ...
                    c_v = MoistThermodynamics.cv_m(q_t, q_l, q_i)
                    PP  = (R_gas/c_v) * (EP - (UP^2 + VP^2 + WP^2)/(2*ρP)- ρP * gravity * zP) 
                    IEP = EP/ρP - 0.5*ρP*(uP^2 + vP^2 + wP^2) - gravity * zP
        ...
        for s = 1:length(RKA)
            ...
            if (step == 1 && (_icase == 1201 || _icase == 1202 || _icase == 1010 ))
                ...
                @inbounds for e = 1:nelem, i = 1:Np
                    z = d_vgeoL[i, _z, e]
                    geopotential = gravity * z
                    U, V, W = d_QL[i, _U, e], d_QL[i, _V, e], d_QL[i, _W, e]
                    ρ, E    = d_QL[i, _ρ, e], d_QL[i, _E, e]
                    u, v, w = U/ρ, V/ρ, W/ρ
                    q_tr[1], q_tr[2], q_tr[3] = d_QL[i, _qt1, e] , 0.0 , 0.0
                    q_t = q_tr[1]
                    T_prev = d_QL[i, _nstate+1, e]
                    KE = 0.5 * (u^2 + v^2 + w^2) 
                    E_int = E/ρ - KE - geopotential
                    
                    # Properties for moist quantities
                    # T_prev is the initial condition for iterations (carried out in MoistThermodynamics)
                    T = MoistThermodynamics.saturation_adjustment(E_int, ρ, q_t/ρ, T_prev)
...
function convert_set2nc_to_set3c(::Val{dim}, ::Val{N}, vgeo, Q) where {dim, N}
    ...
    if (_icase != 1010)
        ...
        return
        @inbounds for e = 1:nelem, n = 1:Np
            ρ, u, v, w, θ = Q[n, _ρ, e], Q[n, _U, e], Q[n, _V, e], Q[n, _W, e], Q[n, _E, e]
            z = vgeo[n, _z, e]
    ...
    q_tr = zeros(DFloat, ntracers)
        ...
        if (_icase!=1010)   
            @inbounds for e = 1:nelem, n = 1:Np
                ...
                w = W/ρ
                E = E/ρ
                P = (R_gas/c_v)*ρ*(E - 0.5*(u^2 + v^2 + w^2) - geopotential)
            ...
            @inbounds for e = 1:nelem, n = 1:Np
                ...
                E     = E/ρ
                E_int = E - 0.5*(u^2 + v^2 + w^2) - geopotential
...
function main()
    ...
    function ic(dim, x...)
     ...
     #       E       = θ
        ...
        elseif (icase == 1001) # --------------------------------------------------------
            # Rising Thermal Bubble with single passive tracer
            u0      = 0
            r = sqrt((x[1]-500)^2 + (x[dim]-350)^2 ) 
            rc      = 250.0
        ...
        elseif (icase == 1010)
            ...
            θ       = θ_ref + Δθ
            exner_k = (1.0 - gravity/(c_p*θ)*x[dim])
            c       = c_v/R_gas
            rho     = p0/(R_gas*θ)*(exner_k)^c
            p       = p0 * (rho * R_gas*θ/p0)^(c_p/ c_v)
            T       = exner_k*θ
            ...
            E_int = c_v*T
            E     = E_int + 0.5*(U^2 + V^2 + W^2) + geopotential
